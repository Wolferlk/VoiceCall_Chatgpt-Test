...
class AahaasAssistentV01TurnController extends Controller
{
        ...
        try {
            ...
            if ($transcript === '') {
                try {
                    ...
                    $call->forceFill(['conversation_history' => $recoveryHistory])->save();
            ...
            if (($mergedProfile['package_state'] ?? '') === 'offered' && $packageFeedback === 'accepted') {
                ...
                if (in_array($packageLookupStatus, ['failed', 'api_unavailable'], true)) {
                    ...
                    $asksForDetail   = array_filter($detailKeywords, fn ($kw) => str_contains($transcriptLower, $kw));

                    if (! isset($mergedProfile['suggested_package']) && $packageLookupStatus !== '' && ! empty($asksForDetail)) {
            ...
            return response()->json([
                'message' => $throwable->getMessage() !== '' ? $throwable->getMessage() : 'Aahaas Assistent V01 call turn failed.',
